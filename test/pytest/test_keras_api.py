...
from tensorflow.keras.layers import (ELU, Activation, Add, Average,
                                     AveragePooling1D, AveragePooling2D,
                                     BatchNormalization, Concatenate, Conv1D,
                                     Conv2D, Dense, Input, LeakyReLU, Maximum,
                                     MaxPooling1D, MaxPooling2D, Minimum,
                                     Multiply, PReLU, Reshape, Subtract,
                                     ThresholdedReLU)
...
def test_activations(activation_function, backend, io_type):
    ...
    output_dir = str(
        test_root_path
        / f'hls4mlprj_keras_api_activations_{activation_function.__class__.__name__}_{backend}_{io_type}'
    )
    hls_model = hls4ml.converters.convert_from_keras_model(
        model, hls_config=config, output_dir=output_dir, backend=backend, io_type=io_type
    )
...
def test_pooling(pooling, padds, chans, backend):
    ...
    output_dir = str(
        test_root_path
        / f'hls4mlprj_keras_api_pooling_{pooling.__name__}_channels_{chans}_padds_{padds}_backend_{backend}'
    )
    hls_model = hls4ml.converters.convert_from_keras_model(
