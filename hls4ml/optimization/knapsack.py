...
def __solve_knapsack_branch_and_bound(values, weights, capacity, time_limit=sys.float_info.max, scaling_factor=10e4):
    ...
    solver = pywrapknapsack_solver.KnapsackSolver(pywrapknapsack_solver.KnapsackSolver.KNAPSACK_MULTIDIMENSION_BRANCH_AND_BOUND_SOLVER, 'BB')
    solver.set_time_limit(time_limit)
    solver.Init((values * scaling_factor).tolist(), weights.tolist(), capacity.tolist())
...
def __solve_knapsack_cbc_mip(values, weights, capacity, time_limit=sys.float_info.max, scaling_factor=10e4):
    ...
    solver = pywrapknapsack_solver.KnapsackSolver(pywrapknapsack_solver.KnapsackSolver.KNAPSACK_MULTIDIMENSION_CBC_MIP_SOLVER, 'CBC')
    solver.set_time_limit(time_limit)
    solver.Init((values * scaling_factor).tolist(), weights.tolist(), capacity.tolist())
    optimal = solver.Solve()
    selected = [i for i in range(values.shape[0]) if solver.BestSolutionContains(i)]
