...
class hls4mlAttributes:
    ...
    '''
    def __init__(self, n_in, n_out, io_type, strategy, weight_precision, output_precision, reuse_factor, parallelization_factor=1):    
...
class OptimizationAttributes:
    ...
    '''
    def __init__(self, structure_type=SUPPORTED_STRUCTURES.UNSTRUCTURED, pruning=False, weight_sharing=False, block_shape=(1, 1), pattern_offset=1, consecutive_patterns=1):
...
class LayerAttributes:
    ...
    '''
    def __init__(self, name, layer_type, inbound_layers, weight_shape, input_shape,output_shape,optimizable, optimization_attributes, args):
        self.name = name
...
def get_attributes_from_keras_model(model):
    ...
    for i, layer in enumerate(model.layers):
        ...
        model_attributes[layer.name] = LayerAttributes(
            layer.name, layer.__class__, inbound_layers,
            weight_shape, layer.input_shape[1:], layer.output_shape[1:],
            False, OptimizationAttributes(), {}
        )
...
def get_attributes_from_keras_model_and_hls4ml_config(model, config):
    ...
    for layer in model_attributes:
        if model_attributes[layer].layer_type in SUPPORTED_LAYERS:
            ...
            weight_precision = layer_config['Precision']['weight'] if 'weight' in layer_config['Precision'] else default_precision
            weight_precision = hls4ml.backends.fpga.fpga_backend.FPGABackend.convert_precision_string(weight_precision)
            output_precision = layer_config['Precision']['result'] if 'result' in layer_config['Precision'] else default_precision
            ...
            hls4ml_attributes = hls4mlAttributes(
                n_in, n_out,
                io_type, strategy,
                weight_precision, output_precision,
                reuse_factor, parallelization_factor
            )
