...
@tf.function
def backprop(model, X, y, loss_fn):
    '''
...
# TODO - Returning a loss causes a crash sometimes [Jupyter Notebook witout kernel restarting]
@tf.function
def masked_backprop(model, X, y, loss_fn, model_attributes, masks):
    '''
    Helper function to train a model with weight freezing during backprop
    ...
    Notes:
        Some optimizers (e.g. Adam) accumulate information from past gradients
        Therefore, using this function stand-alone might not necessarily freeze weights completely
        The optimizer needs should be reset when model masks are changed; see top-level function
        TODO - Is it possible to freeze specific weights in a tensor, regardless of the optimizer?
    '''
    grads = []
    with tf.GradientTape(persistent=True) as tape:
      ...
      for layer in model.layers:
          if layer.trainable_weights:
            ...
            if model_attributes[layer.name].optimizable:
              grad[0] = tf.multiply(grad[0], masks[layer.name])
            grads += grad
    model.optimizer.apply_gradients(zip(grads, model.trainable_weights))
    return loss

def evaluate_model(model, X, y, metric, verbose):
    '''
    Helper function for evaluating a model, given inputs, labels and a user-defined metric

    Args:
        - model (keras.model): Input model
        - metric (keras.metrics.Metric): Metric to be evaluated
        - X (np.array): Input data
        - y (np.array): Output data
        - verbose (boolen): Print outputs
    
    Return:
        - results (np.array): Model performance with input X, output y, relative to metric
    '''
    metric.reset_states()
    outputs = model.predict(X, verbose=verbose)
