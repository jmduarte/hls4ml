...
class DenseRegularizer(tf.keras.regularizers.Regularizer):
    ...
    '''
    def __init__(self, alpha, beta=0, norm=1, structure_type=SUPPORTED_STRUCTURES.UNSTRUCTURED, block_shape=(1, 1), pattern_offset=1, consecutive_patterns=1):
    ...
    def __call__(self, weights):
        ...
        if self.structure_type == SUPPORTED_STRUCTURES.PATTERN:
            ...
            target_shape = (number_of_patterns, self.pattern_offset)
            reshaped = tf.reshape(weights, target_shape)
            ...
            blocks = tf.reshape(
                                tf.image.extract_patches(
                                    ...
                                    [1, number_of_patterns, self.consecutive_patterns, 1],
                                    [1, number_of_patterns, self.consecutive_patterns, 1], 
                                    [1, 1, 1, 1], 'SAME'
                                ),
                                (total_blocks, number_of_patterns * self.consecutive_patterns)
                            )
...
class Conv2DRegularizer(tf.keras.regularizers.Regularizer):
    ...
    '''
    def __init__(self, alpha, beta=0, norm=1, structure_type=SUPPORTED_STRUCTURES.UNSTRUCTURED, pattern_offset=1, consecutive_patterns=1):
        if norm != 1 and norm != 2:
            raise Exception(f'{self.__class__.__name__} currently supports l1- and l2-based regularization')
    ...
    def __call__(self, weights):
        ...
        if self.structure_type == SUPPORTED_STRUCTURES.STRUCTURED:
            sparse_penalty = self.alpha * tf.norm(tf.reduce_sum(tf.norm(weights, axis=(0, 1), ord='fro'), axis=0), ord=self.norm)
