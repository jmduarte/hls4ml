...
def optimize_model(
    ...
    if not isinstance(optimizer, (tf.keras.optimizers.Optimizer, tf.keras.optimizers.legacy.Optimizer)):
        raise Exception(
            'Optimizer must be an instance of keras.optimizers.Optimizer.'
    ...
    if not isinstance(loss_fn, tf.keras.losses.Loss):
        raise Exception(
            'Loss function must be an instance of tensorflow.keras.losses.Loss.'
    ...
    if not isinstance(validation_metric, tf.keras.metrics.Metric):
        raise Exception(
            'Validation metric must be an instance of tensorflow.keras.losses.Loss.'
    ...
    while sparsity_conditions:
        ...
        for epoch in range(epochs - EPOCHS_REWINDING):
            ...
            for i in range(len(batch_X)):
                loss_value = utils.masked_backprop(
                    optimizable_model, batch_X[i], batch_y[i], loss_fn, model_attributes, masks
                )
            ...
            if early_stopping_epochs is not None:
                ...
                if verbose:
                    print(
                        f'Epoch: {epoch + 1} - Time: {time.time() - start_time}s - Training Loss: {round(epoch_loss_avg.result(), 3)} - Validation Loss: {round(val_loss_current, 3)}'
                    )
            else:
                if verbose:
                    print(
                        f'Epoch: {epoch + 1} - Time: {time.time() - start_time}s - Training Loss: {round(epoch_loss_avg.result(), 3)}'
                    )
        ...
        if verbose:
            print(
                f'Performance on validation set: {pruned_performance} with a target sparsity: {target_sparsity} [relative to objective]'
            )
