...
def get_model_masks(keras_model, model_attributes, sparsity, objective, metric='l1', local=False, gradients=None, hessians=None, knapsack_solver='CBC_MIP'):
    '''
    ...
    if local:
        return __get_masks_local(keras_model, model_attributes, sparsity, objective, metric, gradients, hessians, knapsack_solver)
    else:
        return __get_masks_global(keras_model, model_attributes, sparsity, objective, metric, gradients, hessians, knapsack_solver)
    ...
    for layer in keras_model.layers:
        ...
        if isinstance(layer, SUPPORTED_LAYERS) and model_attributes[layer.name].optimizable:
            ...
            if structure_type == SUPPORTED_STRUCTURES.PATTERN:
                ...
                number_of_patterns = np.prod(value.shape) // pattern_offset
                ...
                # Decode masked groups into original shape and set selected groups to one
                mask = np.zeros((np.prod(original_shape), ), value.dtype)
                for i in selected:
                    pos = i * consecutive_patterns
                    for j in range(pos, pos + consecutive_patterns):
                        mask[np.linspace(j, (number_of_patterns - 1) * pattern_offset + j, number_of_patterns, dtype=np.int32).tolist()] = 1 
                
                # Calculate offset
                not_selected = [i for i in range(len(groups)) if i not in selected]
                offset = np.zeros((np.prod(original_shape), ), value.dtype)
                for i in not_selected:
                    if groups[i].optimization_type == 'sharing':
                        mean = np.mean(blocks[i, :])
                        pos = i * consecutive_patterns
                        for j in range(pos, pos + consecutive_patterns):
                            offset[np.linspace(j, (number_of_patterns - 1) * pattern_offset + j, number_of_patterns, dtype=np.int32).tolist()] = mean 
                
                # Reshape into original shape and store result
                mask = np.reshape(mask, original_shape)
                offset = np.reshape(offset, original_shape)
                masks[layer.name] = mask
                offsets[layer.name] = offset
...
def __get_masks_global(keras_model, model_attributes, sparsity, objective, metric, gradients, hessians, knapsack_solver):
    ...
    for layer in keras_model.layers:
        ...
        layer_optimizable = model_attributes[layer.name].optimizable and (model_attributes[layer.name].optimization_attributes.weight_sharing or model_attributes[layer.name].optimization_attributes.pruning)
        if isinstance(layer, SUPPORTED_LAYERS) and layer_optimizable:
            ...
            if structure_type == SUPPORTED_STRUCTURES.UNSTRUCTURED:
                ...
                if model_attributes[layer.name].optimization_attributes.pruning:
                    ...
                    indices = np.indices(value.shape).reshape(value.ndim, -1).T
                    for i in indices:
                        group = __WeightGroups__(value[tuple(i)], layer_savings, tuple(i), structure_type, layer.name, 'pruning')
                        groups.append(group)
            ...
            if structure_type == SUPPORTED_STRUCTURES.STRUCTURED:
                if isinstance(layer, (Dense, QDense)):
                    ...
                    for i in range(vals_norm.shape[0]):
                        if vals_norm[i] <= vals_var[i]:
                            groups.append(__WeightGroups__(vals_norm[i], layer_savings, i, structure_type, layer.name, optimization_type='pruning'))
                        else:
                            groups.append(__WeightGroups__(vals_var[i], layer_savings, i, structure_type, layer.name, optimization_type='sharing'))
    ...
    for layer in keras_model.layers:
        if isinstance(layer, SUPPORTED_LAYERS) and model_attributes[layer.name].optimizable:
            ...
            not_selected_layer = [i for i in range(len(groups)) if groups[i].layer_name == layer.name and i not in selected_layer]
            ...
            if structure_type == SUPPORTED_STRUCTURES.PATTERN:
                ...
                target_shape = (number_of_patterns, pattern_offset)
                reshaped = np.reshape(layer.get_weights()[0], target_shape)
                total_blocks = pattern_offset // consecutive_patterns
                blocks = np.reshape(
                            tf.image.extract_patches(
                                np.expand_dims(np.expand_dims(reshaped, 2), 0),
                ...
                mask = np.zeros((np.prod(model_attributes[layer.name].weight_shape), ), layer.get_weights()[0].dtype)            
                for i in selected_layer:
                    pos = groups[i].layer_position * consecutive_patterns
                    for j in range(pos, pos + consecutive_patterns):
                        mask[np.linspace(j, (number_of_patterns - 1) * pattern_offset + j, number_of_patterns, dtype=np.int32).tolist()] = 1 
                mask = np.reshape(mask, model_attributes[layer.name].weight_shape)
                
                offset = np.zeros((np.prod(model_attributes[layer.name].weight_shape), ), layer.get_weights()[0].dtype)
                for i in not_selected_layer:
                    if groups[i].optimization_type == 'sharing':
                        mean = np.mean(blocks[i, :])
                        pos = groups[i].layer_position * consecutive_patterns
                        for j in range(pos, pos + consecutive_patterns):
                            offset[np.linspace(j, (number_of_patterns - 1) * pattern_offset + j, number_of_patterns, dtype=np.int32).tolist()] = mean 
                offset = np.reshape(offset, model_attributes[layer.name].weight_shape)
                masks[layer.name] = mask
                offsets[layer.name] = offset
