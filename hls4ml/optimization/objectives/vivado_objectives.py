...
class VivadoDSPEstimator(ObjectiveEstimator):
    ...
    def is_layer_optimizable(self, layer_attributes):
        if not layer_attributes.weight_shape or layer_attributes.args['hls4ml_attributes'].weight_precision.width < 9:
            ...
            return True, OptimizationAttributes(SUPPORTED_STRUCTURES.PATTERN, pruning = True, weight_sharing = False,
                                                pattern_offset=np.prod(layer_attributes.weight_shape) // layer_attributes.args['hls4ml_attributes'].reuse_factor, consecutive_patterns=1)
