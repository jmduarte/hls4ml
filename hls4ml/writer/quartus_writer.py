    ...
    def write_testbench_stream(self, model):
        ...
        for line in f.readlines():
            ...
            # TODO - This is one-input specific (are multiple model inputs needed at all?)
                ...
                newline += (
                    indent
                    + f'ihc_hls_enqueue_noret(&{model.config.get_project_name()}, {input_params}, {output_params}); \n'
                )
    ...
    def write_bridge(self, model):
        ...
        for line in f.readlines():
            ...
            if 'MYPROJECT' in line:
                ...
                if io_type == 'io_stream':
                    inputs_str = ', '.join(
                        [f'{dtype} {i.name}[{i.size_cpp()}]' for i in model_inputs]
                    )
                    outputs_str = ', '.join(
                        [f'{dtype} {o.name}[{o.size_cpp()}]' for o in model_outputs]
                    )
                else:
                    inputs_str = ', '.join(
                        [
                            f'{dtype} {i.member_name}[{i.size_cpp()}]'
                            for i in model_inputs
                        ]
                    )
                    outputs_str = ', '.join(
                        [
                            f'{dtype} {o.member_name}[{o.size_cpp()}]'
                            for o in model_outputs
                        ]
                    )

                insize_str = ', '.join(
                    [f'unsigned short &const_size_in_{i}' for i in range(1, len(model_inputs) + 1)]
                )
                outsize_str = ', '.join(
                    [f'unsigned short &const_size_out_{o}' for o in range(1, len(model_outputs) + 1)]
                )
                ...
                if io_type == 'io_stream':
                    ...
                    top_level = indent + f'{model.config.get_project_name()}({input_params}, {output_params});\n'
                    newline += top_level
                    newline += '\n'
                    ...
                    # Execute top-level function
                    top_level = indent + f'outputs_ap = {model.config.get_project_name()}(inputs_ap);\n'
                    newline += top_level
                    newline += '\n'
                ...
                for layer in model.get_layers():
                    ...
                    if func and model.config.trace_output and layer.get_attr('Trace', False):
                        ...
                        for var in vars:
                            newline += (
                                indent
                                + 'nnet::trace_outputs->insert(std::pair<std::string, void *>("{}", (void *) malloc({} * element_size)));\n'.format(
