...
from hls4ml.model.types import (CompressedType, ExponentPrecisionType,
                                ExponentType, FixedPrecisionType,
                                IntegerPrecisionType, NamedType, PackedType,
...
class InplaceVariableConverter:
    ...
    def convert(self, tensor_var, io_type):
        ...
        if io_type == 'io_stream':
            tensor_var.type = self.type_converter.convert(
                PackedType(tensor_var.type.name, tensor_var.type.precision, tensor_var.shape[-1], n_pack=1)
            )
        else:
            tensor_var.type = self.type_converter.convert(tensor_var.type)
