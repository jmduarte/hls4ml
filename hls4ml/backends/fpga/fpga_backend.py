...
class FPGABackend(Backend):
    ...
    def set_closest_reuse_factor(self, layer, n_in, n_out, attribute='reuse_factor'):
        ...
        if chosen_rf not in valid_rf:
            ...
            print(
                'WARNING: Invalid ReuseFactor={} in layer "{}". Using ReuseFactor={} instead. Valid ReuseFactor(s): {}.'.format(
                    chosen_rf, layer.name, closest_rf, ','.join(map(str, valid_rf))
                )
            )
    ...
    def convert_precision_string(cls, precision):
        ...
        if precision.startswith('ap_'):
            ...
            raise Exception(f'Cannot convert precision string: {precision}')
