...
from hls4ml.model.layers import (GRU, LSTM, Activation, BatchNormalization,
                                 Concatenate, Conv1D, Conv2D, Conv2DBatchnorm,
                                 Dense, DepthwiseConv2D, Dot, Embedding,
                                 GarNet, GarNetStack, GlobalPooling1D,
                                 GlobalPooling2D, Layer, Merge,
                                 ParametrizedActivation, Pooling1D, Pooling2D,
                                 PReLU, Resize, SeparableConv1D,
                                 SeparableConv2D, SimpleRNN, Softmax,
                                 Transpose, ZeroPadding1D, ZeroPadding2D)
...
class VivadoBackend(FPGABackend):
    ...
    def build(
        ...
        os.system(
            'vivado_hls -f build_prj.tcl "reset={reset} csim={csim} synth={synth} cosim={cosim} validation={validation} export={export} vsynth={vsynth} fifo_opt={fifo_opt}"'.format(
                reset=reset,
                csim=csim,
                synth=synth,
                cosim=cosim,
    ...
    def init_conv1d(self, layer):
        ...
        if chosen_pf not in valid_pf:
            ...
            print(
                'WARNING: Invalid ParallelizationFactor={} in layer "{}". Using ParallelizationFactor={} instead. Valid ParallelizationFactor(s): {}.'.format(
                    chosen_pf, layer.name, closest_pf, ','.join(map(str, valid_pf))
                )
            )
        else:
