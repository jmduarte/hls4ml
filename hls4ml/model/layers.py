...
from hls4ml.model.attributes import (Attribute, AttributeDict,
                                     AttributeMapping, ChoiceAttribute,
                                     CodeMapping, TypeAttribute, TypeMapping,
...
class GlobalPooling1D(Layer):
    _expected_attributes = [Attribute('n_in'), Attribute('n_filt'), ChoiceAttribute('pool_op', ['Max', 'Average'])]
...
class GarNet(Layer):
    ...
    def initialize(self):
        ...
        if self.attributes['n_vertices'] % reuse_factor != 0:
            raise Exception(
                'GarNet vertex loop has no bound check; number of vertices must be divisible by the reuse factor ({}).'.format(
                    reuse_factor
                )
            )
    ...
    def _initialize_transforms(self):
        ...
        for op_name, lname, wtype in weights_source:
            data = self.model.get_weights_data(
                self.name, f'{self.name}/{lname}_{wtype}:0'
            )
            if wtype == 'kernel':
    ...
    def _make_input_transform_weights(self, n_propagate, n_aggregators, n_out_features, quantize=False, sublayer=''):
        ...
        input_transform_bias = self.model.get_weights_data(
            self.name, f'{self.name}/FLR{sublayer}_bias:0'
        )  # [n_propagate]
        if quantize:
